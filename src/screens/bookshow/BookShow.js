...
class BookShow extends Component {
  ...
  componentWillMount() {
      ...
      function () {
        if (xhrShows.readyState === 4) {
          ...
          for (let show of response.shows) {
            newLocations.push({
              id: show.theatre[0].city,
              location: show.theatre[0].city,
            });
          }

          newLocations = newLocations.filter(
            (loc, index, self) =>
              index === self.findIndex((c) => c.id === loc.id)
          );
    ...
    xhrShows.open(
      "GET",
      this.props.baseUrl + "shows/" + this.props.match.params.id
    );
  ...
  locationChangeHandler = ((event) => {
    ...
    for (let show of this.state.originalShows) {
      if (show.theatre[0].city === event.target.value) {
        newTheatres.push({
          id: show.theatre[0].name,
          theatre: show.theatre[0].name,
        });
      }
    }
  ...
  theatreChangeHandler = ((event) => {
    ...
    for (let show of this.state.originalShows) {
      if (
        show.theatre[0].city === this.state.location &&
        show.theatre[0].name === event.target.value
      ) {
  ...
  languageChangeHandler = ((event) => {
    ...
    for (let show of this.state.originalShows) {
      if (
        show.theatre[0].city === this.state.location &&
        show.theatre[0].name === this.state.theatre &&
  ...
  showDateChangeHandler = ((event) => {
    ...
    for (let show of this.state.originalShows) {
      if (
        show.theatre.city === this.state.location &&
        show.theatre.name === this.state.theatre &&
        show.language === this.state.language &&
  ...
  bookShowButtonHandler = (() => {
    ...
    this.state.theatre === ""
      ? this.setState({ reqTheatre: "dispBlock" })
      : this.setState({ reqTheatre: "dispNone" });
    this.state.language === ""
            ...
            <CardContent>
              ...
              <FormControl required className="formControl">
                <InputLabel htmlFor="tickets">
                  Seat Selection: ( {this.state.availableTickets} available )
                </InputLabel>
                <Input
                  id="tickets"
              ...
              <Typography>
                Total Price: Rs.{" "}
                {this.state.unitPrice * this.state.tickets.length}
              </Typography>
              <br />
              <br />
              <Button
